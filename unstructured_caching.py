    ...
    while 1:
        ...
        if request.split(' ')[1]=='JOIN': #to handle JOIN requests
            ...
            if searchquery in search:
                ...
                query = re.findall('"([^"]*)"', request)[0]
                if searchpop.has_key(query):
                    searchpop[query]+=1
                else:
                    searchpop[query] = 1
                '''if searchcache.has_key(query):
                    pass
                elif len(searchcache)<cache_size:
                    searchcache[query]= None
                    print searchcache
                else:
                    cache_keys = searchcache.keys()
                    cache_pop = [searchpop[cache_keys[i]] for i in range(len(searchcache))]
                    if min(cache_pop)<searchpop[query]:
                        searchcache.pop(cache_keys[np.argmin(cache_pop)])
                        searchcache[query]= None
                        print cache_keys[np.argmin(cache_pop)],searchpop[cache_keys[np.argmin(cache_pop)]],query,searchpop[query]
                        print searchcache'''
            ...
            if request.split(' ')[2] == '1':
                ...
                search_request = re.findall('"([^"]*)"', request)[0]
                ipandport = request.split(' ')[3]+' '+request.split(' ')[4]
                if searchpop.has_key(search_request):
                    if searchcache.has_key(search_request):
                        searchcache[search_request]=ipandport
                        print searchcache
                    elif len(searchcache)<cache_size:
                        searchcache[search_request] = ipandport
                        print searchcache
                        ...
                        cache_keys = searchcache.keys()
                        cache_pop = [searchpop[cache_keys[i]] for i in range(len(searchcache))]
                        if min(cache_pop)<searchpop[search_request]:
                            searchcache.pop(cache_keys[np.argmin(cache_pop)])
                            searchcache[query]= ipandport
                            print cache_keys[np.argmin(cache_pop)],searchpop[cache_keys[np.argmin(cache_pop)]],query,searchpop[query]
                            print searchcache
...
def SearchQuery(search_request): #to search entries for query received
    ...
    query = re.findall('"([^"]*)"', search_request)[0]
    hops = str(int(search_request.split(' ')[4])+1).zfill(2)
    if searchcache.has_key(query):
        if searchcache[query] != None:
            lent = len('SEROK 1')+len(searchcache[query])+len(str(hops))+len(str(search_request.split(' ')[5]))+len(query)+4
            response = str(lent).zfill(4)+' '+'SEROK 1'+' '+searchcache[query]+' '+hops+' '+str(search_request.split(' ')[5])+' '+'"'+query+'"'
            response = [response]
            return response
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s','--cache_size',help='Search cache size(Should be integer only',required=True)
    ...
    max_hops = 10
    cache_size = abs(int(args['cache_size']))
    if len(str(PORT))>5:
    ...
    if action == 'REG':
        ...
        while leave != 'LEAVE': #to take prompts from user while the program is running
            ...
            if leave == 'LEAVE':
                ...
                file = open('80node0.8cache4datajahnavi.csv','a')
                wrfile = csv.writer(file)
                data = []
                data.append(min(latency))
                data.append(max(latency))
                data.append(np.std(latency))
                data.append(np.mean(latency))
                data.append(min(hops))
                data.append(max(hops))
                data.append(np.std(hops))
                data.append(np.mean(hops))
                #number of messages processed
                data.append(len(search))
                routing_size = 0
                for i in range(len(RoutingTable)):
                    for j in range(len(RoutingTable[RoutingTable.keys()[i]])):
                        routing_size += 1
                #Routing table size
                data.append(routing_size)
		data = data+searchcache.keys()
                wrfile.writerow(data)
                file.close()
